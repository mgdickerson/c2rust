...
use rustc::mir::{Local, Body, Mutability};
use rustc::mir::BindingForm::Var;
use rustc::mir::ClearCrossCrate::Set;
use std::collections::{BTreeMap, HashSet};
use std::iter::FromIterator;
use syntax::ast::{BindingMode, Crate, PatKind};
...
impl Transform for RemoveMutability {
    fn transform(&self, krate: &mut Crate, _st: &CommandState, cx: &RefactorCtxt) {
        ...
        mut_visit_fns(krate, |fl| {
            ...
            let used_mut_user_locals: HashSet<Local> = HashSet::from_iter(borrow_check_result.used_mut.iter().cloned());

            let sorted_user_locals = sort_user_locals(&mir);

            for arg in fl.decl.inputs.iter_mut() {
                let arg_span = arg.pat.span.data();
                let mir_local = sorted_user_locals.get(&arg_span).expect("Can't match arg node to local.");
                ...
                arg.pat.node = match &arg.pat.node {
                    PatKind::Ident(BindingMode::ByValue(_), ident, span) => {
                        PatKind::Ident(BindingMode::ByValue(syntax::ast::Mutability::Immutable), *ident, span.clone())
                    }
                    x => { x.clone() }
                };
            }
...
fn sort_user_locals(mir: &Body) -> BTreeMap<SpanData, Local> {
